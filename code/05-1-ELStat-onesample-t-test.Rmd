---
title: 'Übungen: Mittelwertsvergleiche, Teil 1'
...
den Sie bereits in früheren Übungen bearbeitet haben.
...
* Stichprobenumfang, Mittelwert und Standardabweichung für Gewicht (Achtung in **jamovi** `Filter = Geschlecht == "w"`) berechnen.   
...
* Standardfehler für den Mittelwert berechnen (von Hand oder **jamovi**: `S.E. mean` wählen), zur Erinnerung    
...
paste("z-Wert =", z_value)              # z-Wert anzeigen
```
...
paste("p-Wert =", p_value)       # p-Wert anzeigen
```

* Der p-Wert für z = -16 ist extrem klein und kann nicht aus den Z-Tabellen abgelesen werden. Sie können ihn in **jamovi** > Register *Analyses* > *T-tests* > *One Sample T-Test* > Tests: *Student's* > Hypothesis: *Test value* = 67.6 (unsere Nullhypothese!)
...
* New Yorker schlafen im Durchschnitt 7.73 [7.43, 8.03] Stunden. Das 95%-vertrauensintervall schliesst den Vergleichswert (Nullwert) von 8 Stunden ein. Es besteht keine Evidenz für einen Unterschied zwischen der mittleren Schlafdauer von New Yorkern zu 8 Stunden und die Nullhypothese wird nicht verworfen.  
...
2 * pnorm(-1.7532)   # p-Wert für 2-seitige Hypothese berechnen
```
...
2 * pnorm(-4.48)   # p-Wert für 2-seitige Hypothese berechnen
```
...
* Wir können für die Berechnung des p-Werts einfach das Ergebnis aus Übung 3 halbieren oder den p-Wert neu berechnen:

```{r, echo=TRUE}
# Code für jamovi Rj-Editor

pnorm(-4.48)   # p-Wert für 1-seitige Hypothese berechnen
```
...
Wir prüfen diese Frage anhand einer Zufallsstichprobe von 183 Physiotherapie-Studentinnen. Sie finden diese Daten im Datensatz `bmi_phy_w.csv`. Laden Sie diesen Datensatz in **jamovi**; er umfasst nur die Variable `bmi` für 183 Studentinnen; die Variable wird von **jamovi** automatisch als *continuous-decimal* erkannt.
...
* **jamovi** > Reiter *Analyses* > *Exploration* > *Descriptives* > `bmi` in *Variables* einfügen und unter > im Untermenü *Statistics* die gewünschten Kennzahlen auswählen.
...
* Obwohl der QQ-Plot leicht konvex nach unten gekrümmt ist (was auf eine rechtsschiefe Verteilung hinweist) entscheiden wir vorerst auf annähernd normalverteilte Daten (s.a. Lösung 2).    

7. Vertrauensintervalle für $\hat{\mu}_{bmi}$ berechnen.

* **jamovi** > Register *Analyses* > *Exploration* > *Descriptives* > *Mean Dispersion* > bei *Confidence interval for Mean* Häkchen setzen, Vorgabe ist der Wert 95 für ein 95%-CI.
...
* **jamovi** > Register *Analyses* > *T-Tests* > *One Sample T-Test* > `bmi` in *Dependent Variables* einfügen > unter Tests: *Student's* wählen, unter *Hypothesis > Test value* den Nullwert 23.7 eingeben und zweiseitige Hypothese $\neq Test~ value$ wählen.
...
* **jamovi** > Register *Analyses* > *T-Tests* > *One Sample T-Test* > `bmi` in *Dependent Variables* einfügen > unter Tests: *Wilcoxon rank* wählen > unter *Hypothesis > Test value* den Nullwert 23.7 eingeben und zweiseitige Hypothese $\neq  Test~ value$ wählen.
